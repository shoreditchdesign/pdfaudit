...
from app.services.report_service import REPORT_COLUMNS, ReportBuilder, WorkbookTemplateResolver, compute_summary
...
def test_report_builder_returns_workbook_bytes() -> None:
    ...
    assert workbook.sheetnames == ["Summary", "WCAG", "PDF UA + Semantics", "Remediation", "Source Retrieval"]
    assert sheet.max_row == 2
    assert sheet["A1"].value == "PDF Name"
    assert sheet["A2"].hyperlink.target == "https://example.com/test.pdf"
    assert sheet["B2"].hyperlink.target == "https://example.com/test.pdf"
    assert sheet["E2"].value == "PASS"
    assert sheet["F2"].value == "Pass"
...
def test_report_builder_renders_review_and_pending_labels() -> None:
    ...
    assert sheet.cell(2, header_index["WCAG Reults"]).value == "Needs Review"
    assert "human review" in sheet.cell(2, header_index["WCAG Notes"]).value
    assert sheet.cell(3, header_index["WCAG Reults"]).value == "Pending Adobe"
    assert "Re-run the Adobe step" in sheet.cell(3, header_index["WCAG Notes"]).value
...
def test_report_builder_maps_adobe_findings_into_wcag_remediation_notes() -> None:
    ...
    workbook = load_workbook(BytesIO(report))
    remediation_sheet = workbook["Remediation"]
    remediation_headers = [cell.value for cell in remediation_sheet[1]]
    remediation_index = {str(name): idx + 1 for idx, name in enumerate(remediation_headers) if name is not None}
    remediation_notes = remediation_sheet.cell(2, remediation_index["Remediation Notes"]).value
    ...
    headers = [cell.value for cell in sheet[1]]
    header_index = {str(name): idx + 1 for idx, name in enumerate(headers) if name is not None}
    assert sheet.cell(2, header_index["WCAG Criteria Affected"]).value
    assert "Color contrast" in sheet.cell(2, header_index["WCAG Adobe Findings"]).value
...
def test_report_builder_loads_cached_adobe_findings_for_wcag_columns(tmp_path) -> None:
    ...
    headers = [cell.value for cell in sheet[1]]
    header_index = {str(name): idx + 1 for idx, name in enumerate(headers) if name is not None}
    assert "WCAG 1.4.3 Contrast (Minimum)" in sheet.cell(2, header_index["WCAG Criteria Affected"]).value
    assert "Color contrast" in sheet.cell(2, header_index["WCAG Adobe Findings"]).value
